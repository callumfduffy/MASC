...
class NetInvMgmtMasterEnv(gym.Env):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.rawmat = [
            j for j in self.graph.nodes() if len(list(self.graph.predecessors(j))) == 0
        ]
        ...
        assert set(self.graph.nodes()) == set.union(
            set(self.market), set(self.distrib), set(self.factory), set(self.rawmat)
    ...
    def _RESET(self):
        ...
        self.X = pd.DataFrame(
            data=np.zeros([T + 1, J]), columns=self.main_nodes
        )  # inventory at the beginning of each period
        self.Y = pd.DataFrame(
            data=np.zeros([T + 1, PS]),
    ...
    def _update_state(self):
        ...
        if self.period == 0:
            _pipeline = [[self.Y[k].iloc[0]] for k, v in self.lead_times.items()]
        else:
            _pipeline = [
                self.Y[k].iloc[max(self.period - v, 0) : self.period].values
    ...
    def _STEP(self, action):
        ...
        for key in action.keys():
            ...
            if supplier in self.rawmat:
                self.R.loc[
                    t, (supplier, purchaser)
                ] = request  # accept request since supply is unlimited
                self.S.loc[t, (supplier, purchaser)] = request
            elif supplier in self.distrib:
                X_supplier = self.X.loc[
                    t, supplier
                ]  # request limited by available inventory at beginning of period
                ...
                X_supplier = self.X.loc[
                    t, supplier
                ]  # on-hand inventory at beginning of period
        ...
        for j in self.main_nodes:
            ...
            for k in self.graph.predecessors(j):
                ...
                self.Y.loc[t + 1, (k, j)] = (
                    self.Y.loc[t, (k, j)] - delivery + self.R.loc[t, (k, j)]
                )
        ...
        for j in self.main_nodes:
            ...
            if j not in self.rawmat:
                HC = self.graph.nodes[j]["h"] * self.X.loc[t + 1, j] + np.sum(
                    [
                        self.graph.edges[(k, j)]["g"] * self.Y.loc[t + 1, (k, j)]
                        for k in self.graph.predecessors(j)
                    ]
                )  # holding costs
            else:
...
env = NetInvMgmtMasterEnv(user_D={(1, 0): [10, 10, 10, 10, 10, 10, 10, 10, 10, 10]})
obs = env.reset()
rewards = []
for i in range(10):
    o, r, d, _ = env.step(a)
